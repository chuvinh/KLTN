...
class PageController extends Controller
{
    ...
    public function getInfoUser($id){
     ...
     ->join('user_bill','user_bill.id_user','=','users.char_name')
     ->join('bills','user_bill.id_bill','=','bills.id')
     ->join('customer','customer.id','=','bills.id_customer')
     ->select('bills.id as bills_id','bills.total','bills.date_order','customer.cusname','user_bill.value','users.full_name','bills.status')
     ->get();
     return view('page.detailuser',compact('user'));
 }
...
public function postCheckout(Request $req){
    ...
    $userbill->id_user = $req->iduser;
    $userbill->id_bill = $bill->id;
    $userbill->value = ($cart->totalPrice) * 1 / 100;
    $userbill->save();
